		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pginput===true) {
				$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
					...
					if(key === 13) {
						/*  custom 18.10.05 jqgrid page enter error 수정
						 *  무슨 숫자를 입력하더라도 무조건 1page 되는 오류
						 */
						//if(!clearVals('user')) { return false; }
						let tspg = 'sp_1'+ (ts.p.pager ? "_"+$.jgrid.jqID(ts.p.pager.substr(1)) : "");

						// 최대 page 보다 높은 수 입력시 1page로 돌아감
						if(parseInt($(this).val()) > parseInt($('#'+tspg).text())){
							$(this).val(1);
						}
						// custom end...
