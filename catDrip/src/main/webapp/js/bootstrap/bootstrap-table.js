...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        formatRecordsPerPage: function (pageNumber) {
        	/*return sprintf('%s', pageNumber);*/
        	return sprintf('%s records', pageNumber);
        },
        formatShowingRows: function (pageFrom, pageTo, totalRows) {
        	/*return sprintf('총 %s건 중 %s ~ %s', totalRows, pageFrom, pageTo);*/
    ...
    BootstrapTable.prototype.initData = function (data, type) {//Flowering append와 헷갈릴 소지가 있어 수정함..
    	var that = this; //Flowering
        if (type == 'append') {
            this.data = this.data.concat(data);
        }else if(type == 'prepend'){ //Flowering prepend 추가
        	this.data = data.concat(that.data);
        } else {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        $keepClick = this.$toolbar.find('.dropdown-toggle').removeAttr('data-toggle');//Flowering 추가
        $keepClick.off('click').on('click', function(){//Flowering 추가
        	var target = $(this).parent('.btn-group');
        	var bool = $(target).hasClass('open');
        	if(bool){
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        var that = this,
            ...
            $next, $last,
            $number,
            $pageNumClick,//Flowering 추가 리스트 개수 클릭
        ...
        $pageNumClick = this.$pagination.find('.dropdown-toggle').removeAttr('data-toggle');//Flowering 추가 리스트 개수 클릭
        ...
        $pageNumClick.off('click').on('click', function(){
        	var target = $(this).parent('.btn-group');
        	var bool = $(target).hasClass('open');
        	if(bool){
        		$(target).removeClass('open');
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if(params.pageSize <1 ){
                	params.pageSize = 1;
                	that.options.pageSize = 1;
                }
                
                if(data.total != undefined && data.total > 0 &  data.rows != undefined && data.rows.length == 0){
    ...
    BootstrapTable.prototype.resetHeader = function () {
       ...
       //setTimeout(function () {
            ...
            that.$body.find('tr:first-child:not(.no-records-found) > *').each(function(i) {
                ...
                that.$header_.find('div.fht-cell').eq(i).width($(this).innerWidth()-tailNum);
            });
    ...
    BootstrapTable.prototype.append = function (data) {
        ...
        this.initData(data, 'append'); //Flowering prepend 랑 분기를 위해 변경
        this.initSearch();
        this.initPagination();
        this.initBody(true);
    };
    
    BootstrapTable.prototype.prepend = function (data) { //Flowering 새 함수 추가 append는 뒤에만 추가 되서 앞에쪽에도 추가될수있도록 수정
    ...
    BootstrapTable.prototype.setHeight = function (params) { //Flowering
        this.options.height = params;
        //this.resetHeader();
    };
    
    BootstrapTable.prototype.reWriteHeader = function () { //Flowering
        this.resetHeader();
    };
    
    BootstrapTable.prototype.changePageSize = function (params) { //Flowering
        this.options.pageSize = params;
        this.options.pageNumber = 1;//사이즈가 바뀌어버리면 1페이지로 이동된다.
        this.initServer(params && params.silent);
    };
    ...
    $.fn.bootstrapTable = function (option, _relatedTarget) {
    	/*
    	 * 커스터마이징
    	 * 안재윤 : setHeight 추가 높이를 변경하는 기능. 2014/12/18
    	 * 안재윤 : changePageSize : 페이지 사이즈를 외부에서 변경하는 기능. 2014/12/18
    	 * 안재윤 : reWriteHeader : 단순히 resetHeader만 함
    	 * */
        var allowedMethods = [
                ...
                'setHeight', 'changePageSize', 'reWriteHeader',  'prepend'
            ],
            value;
        ...
        this.each(function () {
            ...
            //Flowering 리사이즈 후 header 위치 다시 설정.
           $(window).bind('resize', function(e)
    		{
		        ...
		        {
		        	//$this.bootstrapTable('reWriteHeader');	
